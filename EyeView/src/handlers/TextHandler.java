 ...
 * javafx.scene.lauput.StackPane. The text can be passed directly or read from 
 * a file. 
 * 
...
public class TextHandler {
	 ...
	 * 
	 * @param defaultColor Default color for text 
	 * @param defaultFont Default font for text
	 */
	...
	/**
	 * Adds a text element with text from the specified file. Will display 
	 * it at the specified points, wrapped (0-1 relative to the size of the 
	 * pane). If the file cannot be found, an IOException will be thrown. Uses
	 * the object's defaults for font and color, with no timeout.
	 * 
	 * @param filename			Name of file containing text to display.
	 ...
	 * disables wrapping.
	 * @throws IOException		If the file cannot be found, or another IO error
	 * occurs when reading the file, this exception will be thrown.
	 */
	 ...
	 * Adds a text element with text from the specified file. Will display 
	 * it at the specified points, wrapped (0-1 relative to the size of the 
	 * pane). Uses the specified font and color. If the file cannot be found, an 
	 ...
	 * @param timeout			Time for which text will be displayed
	 * 
	 * @throws IOException		If the file cannot be found, or another IO error
	 * occurs when reading the file, this exception will be thrown.
	 */
	public void addTextElementByFile(String filename, double xpos, double ypos,
		...
		addTextElement(
				content, xpos, ypos, wrappingWidth, color, font, timeout);
	}
	
	/**
	 * Adds a text element that will be displayed 
	 * at the specified points, wrapped (0-1 relative to the size of the 
	 * pane). Uses the object's defaults for font and color, with no timeout.
	 * 
	...
	public void addTextElement(String text, double xpos, double ypos, 
		...
		addTextElement(text, xpos, ypos, wrappingWidth, 
				defaultColor, defaultFont, 0);
	}
	 ...
	 * the specified pane. 
	 * 
	 * @param pane	StackPane to display objects on. 
	 */
