...
public class ImageGallery extends Window {
	...
	private static final double fitWidth = 410;
	...
	public void createImageGallery(ImageElement image) {
		...
		for (int i = 0; i < 5; i++) {
			galleryImages[i] = new Image("file:./resources/images/house" + (i + 1) + ".jpg", false);
		}
		
		Rectangle rect1 = new Rectangle();
		rect1.setX(fitWidth + xPosition);
		rect1.setY(yPosition);
		rect1.setWidth(xResolution - (fitWidth + xPosition));
		rect1.setHeight(yResolution);
	    rect1.setFill(Color.WHITE);
	    
		Rectangle rect2 = new Rectangle();
		rect2.setX(0);
		rect2.setY(yPosition);
		rect2.setWidth(xPosition);
		rect2.setHeight(yResolution);
	...
	public static class GalleryPictures extends Region {
		...
		public void changeImage(int i, ImageElement image){
			...
			root.getChildren().remove(mainHouseImage);
			
			mainImage = new Image("file:./resources/images/house" + (i + 1) + ".jpg");
			mainHouseImage = new ImageView(mainImage);
		...
		protected void layoutChildren() {
			...
			// keep centered centered
				...
				centered.setLayoutX((SPACING * 2.5) + (SPACING - smallHouseWidth));
			} else {
				centered.setLayoutX((fitWidth/2) + (1.5 * (SPACING - smallHouseWidth)));
			}
			
			//centered.setLayoutX(fitWidth/2);
			centered.setLayoutY(fitHeight + yPosition + scrollbarHeight); // Change to main image + amount
